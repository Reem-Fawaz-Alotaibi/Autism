...
def activity_list(request):
    ...
    visual = Activity.objects.filter(
        category='visual',
        is_active=True
    )

    sensory = Activity.objects.filter(
        category='sensory',
        is_active=True
    )

    motor = Activity.objects.filter(
        category='motor',
        is_active=True
    )

    language = Activity.objects.filter(
        category='language',
        is_active=True
    )
...
def activity_play(request, activity_id):
    ...
    return render(
        request,
        f'activity/{activity.activity_file}',
        {
            'activity': activity
        }
    )